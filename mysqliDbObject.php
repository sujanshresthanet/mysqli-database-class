 ...
 *
 * @method int count ()
 * @method mysqliDbObject ArrayBuilder()
 * @method mysqliDbObject JsonBuilder()
 * @method mysqliDbObject ObjectBuilder()
 ...
 * @method mysqliDbObject query($query, $numRows = null)
 * @method mysqliDbObject rawQuery($query, $bindParams = null)
 * @method mysqliDbObject join(string $objectName, string $key, string $joinType, string $primaryKey)
 * @method mysqliDbObject with(string $objectName)
 * @method mysqliDbObject groupBy(string $groupByField)
 * @method mysqliDbObject orderBy($orderByField, $orderbyDirection = "DESC", $customFieldsOrRegExp = null)
 * @method mysqliDbObject where($whereProp, $whereValue = 'DBNULL', $operator = '=', $cond = 'AND')
 * @method mysqliDbObject orWhere($whereProp, $whereValue = 'DBNULL', $operator = '=')
 * @method mysqliDbObject having($havingProp, $havingValue = 'DBNULL', $operator = '=', $cond = 'AND')
 * @method mysqliDbObject orHaving($havingProp, $havingValue = null, $operator = null)
 * @method mysqliDbObject setQueryOption($options)
 * @method mysqliDbObject setTrace($enabled, $stripPrefix = null)
 * @method mysqliDbObject withTotalCount()
 * @method mysqliDbObject startTransaction()
 * @method mysqliDbObject commit()
 * @method mysqliDbObject rollback()
 * @method mysqliDbObject ping()
 * @method string getLastError()
 * @method string getLastQuery()
 */
class mysqliDbObject {
    /**
     * Working instance of MysqliDbClass created earlier
     *
     * @var MysqliDbClass
     */
    private $db;
    ...
    public function __construct ($data = null) {
        $this->db = MysqliDbClass::getInstance();
	...
	if (isset ($this->data[$name]) && $this->data[$name] instanceof mysqliDbObject)
    ...
    /**
     * Helper function to create mysqliDbObject with Json return type
     *
     * @return mysqliDbObject
     */
    ...
    /**
     * Helper function to create mysqliDbObject with Array return type
     *
     * @return mysqliDbObject
     */
    ...
    /**
     * Helper function to create mysqliDbObject with Object return type.
    ...
    public static function table ($tableName) {
        ...
        if (!class_exists ($tableName))
            eval ("class $tableName extends mysqliDbObject {}");
     ...
     *
     * @return mysqliDbObject|array
     */
    private function byId ($id, $fields = null) {
        $this->db->where (MysqliDbClass::$prefix . $this->dbTable . '.' . $this->primaryKey, $id);
     ...
     * @param array|string $fields Array or coma separated list of fields to fetch
     *
     * @return array Array of mysqliDbObjects
     */
    ...
    private function join ($objectName, $key = null, $joinType = 'LEFT', $primaryKey = null) {
        ...
        if (!$primaryKey)
            $primaryKey = MysqliDbClass::$prefix . $joinObj->dbTable . "." . $joinObj->primaryKey;

        if (!strchr ($key, '.'))
            $joinStr = MysqliDbClass::$prefix . $this->dbTable . ".{$key} = " . $primaryKey;
        else
            $joinStr = MysqliDbClass::$prefix . "{$key} = " . $primaryKey;
     ...
     *
     * Transparently creating mysqliDbObject class to provide smooth API like name::get() name::orderBy()->get()
     *
     * @param string $method
     * @param mixed $arg
     *
    ...
    public function toArray () {
        ...
        foreach ($data as &$d) {
            if ($d instanceof mysqliDbObject)
                $d = $d->data;
        }
        return $data;
    }
    ...
    private function prepareData () {
        ...
        foreach ($this->data as $key => &$value) {
            ...
            if ($value instanceof mysqliDbObject && $value->isNew == true) {
                $id = $value->save();
                if ($id)
                    $value = $id;
                else
    ...
    private static function mysqliDbObjectAutoload ($classname) {
     ...
     *
     * Calling autoload() without path will set path to mysqliDbObjectPath/models/ directory
     *
     * @param string $path
     */
    public static function autoload ($path = null) {
        ...
        spl_autoload_register ("mysqliDbObject::mysqliDbObjectAutoload");
    }
}
