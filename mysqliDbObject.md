mysqliDbObject - model implementation on top of the MysqliDbClass.
...
Include mysqlidb and mysqliDbObject classes. If you want to use model autoloading instead of manually including them in the scripts use `autoload()` method.
```php
require_once("libs/MysqliDbClass.php");
require_once("libs/mysqliDbObject.php");

// db instance
$db = new Mysqlidb('localhost', 'user', '', 'testdb');
// enable class autoloading
mysqliDbObject::autoload("models");
```

Each database table could be easily mapped into a mysqliDbObject instance.  If you do not want to create model for a simple table its object could be simply created with a `table()` method.
```php
$user = mysqliDbObject::table("users");
```

Otherwise basic model should be declared as:
```php
class user extends mysqliDbObject {}
```
...
```php
//$users = mysqliDbObject::table('users')->get();
$users = user::get();
foreach ($users as $u) {
  echo $u->login;
}
...
```php
//$user = mysqliDbObject::table('users')->byId(1);
$user = user::byId(1);
echo $user->login;
```

mysqliDbObject will also assume that each table has a primary key column named "id". You may define a primaryKey property to override this assumption.
...
### Relations
Currently mysqliDbObject supports only `hasMany` and `hasOne` relations. To use them declare `$relations` array in the model class.
...
### Array Fields
mysqliDbObject can automatically handle array type of values. Optionaly you can store arrays in json encoded or in pipe delimited format.
...
Before saving and updating the row, mysqliDbObject does input validation. In case validation rules are set but their criteria is
...
mysqliDbObject can return its data as array instead of object. To do that, the `ArrayBuilder()` function should be used in the beginning of the call.
```php
...
Together with `ArrayBuilder()` and `ObjectBuilder()`, mysqliDbObject can also return a result in json format to avoid extra coding.
```php
...
Please look for a use examples in <a href='tests/mysqliDbObjectTests.php'>tests file</a> and test models inside the <a href='tests/models/'>test models</a> directory
